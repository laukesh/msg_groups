...
class DesignPackageController extends Controller
{
    ...
    {
        $baseQuery = DesignPackage::query()
            ->where('project_id', $project->id);

        $totalPackages = (clone $baseQuery)->count();

        $draftPackages = (clone $baseQuery)
            ->where('status', 'Draft')
            ->count();

        $submittedPackages = (clone $baseQuery)
            ->where('status', 'Submitted')
            ->count();

        $approvedPackages = (clone $baseQuery)
            ->where('status', 'Approved')
            ->count();

        $rejectedPackages = (clone $baseQuery)
        ...
        return view('design-management.packages.index', [
            ...
            'submittedPackages' => $submittedPackages,
