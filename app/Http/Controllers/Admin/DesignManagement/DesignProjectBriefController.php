...
class DesignProjectBriefController extends Controller
{
    ...
    protected function workflowEditRoute(
        Project $project,
        Model $record
    ): string {
        return route(
    ...
    public function index(Project $project): View
    {
        ...
        */
        ...
        $submittedBriefs = $briefs
            ->where('status', 'Submitted')
            ->count();

        $approvedBriefs = $briefs
            ->where('status', 'Approved')
            ->count();

        $rejectedBriefs = $briefs
        ...
        return view(
            'design-management.briefs.index',
            compact(
                'project',
                'briefs',
                'totalBriefs',
    ...
    public function create(Project $project): View
    {
        ...
        return view(
            'design-management.briefs.create',
            [
                'project' => $project,
    ...
    public function store(
        ...
        $this->initializeWorkflowOnCreate(
            $validated,
        ...
        if (empty($validated['version'])) {
            ...
            $validated['version'] =
                $validated['version_number'] . '.0';
        }

        if (
            blank(
                $validated['brief_code'] ?? null
            )
        ) {
        ...
        $brief =
            DesignProjectBrief::create(
                $validated
            );

        $brief->recordWorkflowHistory(
            'Created',
            null,
            $brief->status,
    ...
    public function show(
        ...
        return view(
            'design-management.briefs.show',
            array_merge(
                compact(
                    'project',
                    'brief'
                ),
                $this->workflowViewData(
                    $project,
                    $brief
                )
            )
    ...
    public function edit(
        ...
        $this->ensureWorkflowEditable(
            $brief
        );

        return view(
            'design-management.briefs.edit',
            compact(
                'project',
                'brief'
            )
    ...
    public function update(
        ...
        return redirect()
            ...
            ->with(
                'success',
                'Design brief updated successfully.'
            );
    }
    ...
    public function destroy(
        ...
        return redirect()
            ->route(
                'admin.projects.design-management.briefs.index',
                $project
            )
            ->with(
                'success',
                'Design brief deleted successfully.'
            );
    }
     ...
     */
    protected function validateBrief(
        Request $request,
        ?DesignProjectBrief $brief = null
    ): array {

        return $request->validate([

            'brief_code' =>
                'nullable|string|max:100|unique:design_project_briefs,brief_code,' .
                ($brief?->id ?? 'NULL'),
